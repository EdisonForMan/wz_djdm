<template>
	<div class="custom-document">
		...
		<el-tabs v-model="activeTabsPane" type="card" @tab-click="tabsPaneClick" class="my-tabs">
		    <el-tab-pane v-for="(value, index) of tabsPane" :key="index" :label="value.label" :name="value.name"></el-tab-pane>
		</el-tabs>
		<div class="custom-document-content">
			<el-menu active-text-color="#000" text-color="#000" class="my-menu">
				<el-submenu v-for="(value, index) of tabsMenuData[activeTabsPane]" v-if="value.children && value.children.length > 0" :key="index" :index="index + ''">
					<template slot="title">
						<el-checkbox v-model="value.check" class="my-checkbox" @change="changeCheckboxHandler(index)"></el-checkbox>
						<span>{{ value.name }}</span>
						<span v-if="value.innerText">{{ value.innerText }}</span>
					</template>
					<el-menu-item :index="index + '-' + ind" v-for="(item, ind) of value.children" :key="ind" @click="menuItemClickHandler(item)" class="children-menu-item">
						<el-checkbox v-model="item.check" class="my-checkbox" @change="changeCheckboxHandler(index, ind)" v-show="false"></el-checkbox>
						{{ item.name }}
					</el-menu-item>
				</el-submenu>
...
export default {
	...
	methods: {
		tabsPaneClick (tabName) {
			console.log(this.activeTabsPane);
		},
			...
			if (childrenIndex !== null && childrenIndex !== undefined) {
				...
				currentMenu[parentIndex].children.map((item, index) => {
					if (index === childrenIndex) {
						item.check ? this.$emit('document-single-choose', item) : this.$emit('document-single-close', item);
					}
				...
				this.$set(currentMenu[parentIndex], 'check', parentCheck); // 强制渲染
				parentCheck ? this.$emit('document-single-list-choose', items) : this.$emit('document-single-list-close', items);
			}
		},
		// 单独点击一个侧目录的子项
		menuItemClickHandler (obj) {
			this.$emit('menu-item-click', obj);
		},
		// 查询
		query () {
			this.$emit('query-handler', this.queryValue);
		}
	},
	mounted () {
	}
...
<style scoped>
	...
	.custom-document-content::-webkit-scrollbar{/*外层轨道，可以用display:none让其不显示，也可以添加背景图片，颜色改变显示效果（位置3）*/
        height: 4px;
        width: 4px;
    }
    .custom-document-content::-webkit-scrollbar-track-piece{/*内层轨道，滚动条中间部分（位置4）*/
        background:rgba(4,30,117,0.4);
        border-radius:2px;
    }
    .custom-document-content::-webkit-scrollbar-thumb{ /*滚动条里面可以拖动的那部分（位置5）*/
        background:rgba(62,158,255,.8);
        border-radius:2px;
    }
	...
	.my-menu /deep/ .el-checkbox__input.is-checked .el-checkbox__inner, .my-menu /deep/ .el-checkbox__input.is-indeterminate .el-checkbox__inner {
